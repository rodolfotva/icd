    ...
    switch (locale) {
      ...
      case "pt":
        ...
        groupPtRepo.findByCategoryid(categoryId).stream().forEach(grp -> {
          temp.add(new Group(grp.getObjectId(), grp.getId(), grp.getName(), grp.getDescription(), grp.getSubGroups()));
        });
        return temp;
      case "fr":
        List<Group> tempFr = new LinkedList<Group>();
        groupFrRepo.findByCategoryid(categoryId).stream().forEach(grp -> {
          tempFr.add(new Group(grp.getObjectId(), grp.getId(), grp.getName(), grp.getDescription(), grp.getSubGroups()));
        });
        return tempFr;
      default:
        return groupRepo.findByCategoryid(categoryId);
