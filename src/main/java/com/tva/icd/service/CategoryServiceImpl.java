    ...
    switch (locale) {
      ...
      case "pt":
        ...
        categoryPtRepo.findByChapterid(chapterId).stream().forEach(cat -> {
          temp.add(new Category(cat.getObjectId(), cat.getId(), cat.getFirst(), cat.getLast(), cat.getTitle(), cat.getChapterId()));
        });
        return temp;
      case "fr":
        List<Category> tempFr = new LinkedList<Category>();
        categoryFrRepo.findByChapterid(chapterId).stream().forEach(cat -> {
          tempFr.add(new Category(cat.getObjectId(), cat.getId(), cat.getFirst(), cat.getLast(), cat.getTitle(), cat.getChapterId()));
        });
        return tempFr;
      default:
        return categoryRepo.findByChapterid(chapterId);
